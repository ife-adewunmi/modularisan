      ...
      try {
        ...
        switch (source.toLowerCase()) {
          ...
          case 'vue':
            ...
            break
          default:
            logError(`Unknown migration source: ${source}. Available: from-nextisan, from-create-react-app, from-angular-cli, from-vue-cli`)
            break
        }
...
async function migrateFromNextisan(rootDir: string, options: any): Promise<void> {
  ...
  for (const configFile of legacyConfigs) {
    ...
    if (await fs.pathExists(configPath)) {
      ...
      try {
        ...
        legacyConfig = configFile.endsWith('.json') ? JSON.parse(content) : require('yaml').parse(content)
        break
      } catch (error) {
    ...
    const { confirm } = await inquirer.prompt([
      {
        type: 'confirm',
        name: 'confirm',
        message: 'This will modify your project structure. Continue with migration?',
        default: false
      }
    ])
  ...
  console.log(`  1. Review the new configuration: ${chalk.green('misan config show')}`)
  console.log(`  2. List your modules: ${chalk.green('misan list modules')}`)
  console.log(`  3. Create new modules: ${chalk.green('misan create:module <name>')}`)
}
...
async function migrateFromAngular(rootDir: string, options: any): Promise<void> {
...
async function showMigrationPlan(analysis: any, legacyConfig: any): Promise<void> {
  console.log(chalk.cyan('Migration Plan:\n'))
...
async function performNextisanMigration(rootDir: string, analysis: any, legacyConfig: any): Promise<void> {
  ...
  const modulesDir = path.join(rootDir, projectStructure.framework.paths.modules)
