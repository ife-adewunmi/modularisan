...
export function createModuleCommand(program: Command): void {
  program
    ...
    .option('-t, --template <template>', 'Module template (basic, full, api, ui)', 'basic')
    ...
    .option('--components <components>', 'Comma-separated list of components to include')
    ...
    .action(async (name, options) => {
      try {
        ...
        if (!name) {
          ...
          logError('Invalid module name. Use kebab-case (e.g., user-management)')
          return
        }
          ...
          const answers = await inquirer.prompt([
            ...
            },
            {
              ...
              choices: [
                ...
                { name: 'Full (components, services, types, hooks, utils, tests)', value: 'full' },
                { name: 'API (services, types, controllers, middleware)', value: 'api' },
        ...
        console.log(`  Components: ${chalk.green(moduleStructure.components.join(', '))}`)
        console.log(`  Routing: ${chalk.green(moduleStructure.hasRouting ? 'Yes' : 'No')}`)
        console.log(`  API: ${chalk.green(moduleStructure.hasApi ? 'Yes' : 'No')}`)
        console.log(`  Tests: ${chalk.green(moduleStructure.hasTests ? 'Yes' : 'No')}`)
        console.log(`  Standalone: ${chalk.green(moduleStructure.isStandalone ? 'Yes' : 'No')}`)
        console.log('')
        console.log(chalk.cyan('Next steps:'))
        console.log(`  1. Add components: ${chalk.green(`misan create:component login-form ${name}`)}`)
        console.log(`  2. Add services: ${chalk.green(`misan create:service user-service ${name}`)}`)
        console.log(`  3. View module: ${chalk.green(`ls ${moduleStructure.path}`)}`)
