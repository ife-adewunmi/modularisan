...
export function aiAnalyzeCommand(program: Command): void {
  program
    ...
    .option('-f, --format <format>', 'Output format (table, json, detailed)', 'table')
    ...
    .action(async (file, options) => {
      try {
        ...
        console.log(`\\n${chalk.bold('Overall Score:')} ${chalk[scoreColor](analysis.score)}/100`)
        
        // Metrics
        if (options.metrics) {
          console.log(`\\n${chalk.bold('Detailed Metrics:')}`)
          console.log(`  Complexity: ${analysis.metrics.complexity}/10`)
          console.log(`  Maintainability: ${analysis.metrics.maintainability}/100`)
        ...
        if (analysis.issues.length > 0) {
          ...
          analysis.issues.forEach((issue, index) => {
            ...
            const typeIcon = issue.type === 'error' ? '❌' : issue.type === 'warning' ? '⚠️' : '💡'
            
            console.log(`  ${index + 1}. ${typeIcon} ${chalk[severityColor](issue.message)}`)
            if (issue.line) {
        ...
        if (analysis.suggestions.length > 0) {
          ...
          analysis.suggestions.forEach((suggestion, index) => {
            const impactColor = suggestion.impact === 'high' ? 'red' : suggestion.impact === 'medium' ? 'yellow' : 'green'
            const effortColor = suggestion.effort === 'low' ? 'green' : suggestion.effort === 'medium' ? 'yellow' : 'red'
            
            console.log(`  ${index + 1}. ${suggestion.message}`)
            console.log(`     Impact: ${chalk[impactColor](suggestion.impact)} | Effort: ${chalk[effortColor](suggestion.effort)}`)
