...
export function initCommand(program: Command): void {
  program
    ...
    .option('--standalone', 'Enable standalone modules (each with package.json)')
      ...
      try {
          ...
          const { overwrite } = await inquirer.prompt([
            {
              type: 'confirm',
              name: 'overwrite',
              message: 'Modularisan is already initialized. Do you want to overwrite the configuration?',
              default: false
            }
          ])
        ...
        try {
          ...
          if (options.framework) {
            const supportedFrameworks = FrameworkDetector.getSupportedFrameworks()
            if (!supportedFrameworks.includes(options.framework)) {
              throw new Error(`Unsupported framework: ${options.framework}. Supported: ${supportedFrameworks.join(', ')}`)
            }
        ...
        if (!options.yes) {
          const answers = await inquirer.prompt([
            {
              ...
              message: 'Enable standalone modules (each module has its own package.json)?',
          ...
          if (answers.createDirectories) {
            await createBasicDirectoryStructure(rootDir, projectStructure, answers.modulesPath)
          }
        } else {
          // Use default options
          configOptions = {
          ...
          await createBasicDirectoryStructure(rootDir, projectStructure, projectStructure.framework.paths.modules)
        }

        // Initialize configuration
        const config = await configManager.initializeConfig(projectStructure, configOptions)
        ...
        console.log(`  TypeScript: ${chalk.green(config.features.typescript ? 'Yes' : 'No')}`)
        console.log(`  Testing: ${chalk.green(config.features.testing ? 'Yes' : 'No')}`)
        console.log(`  Standalone Modules: ${chalk.green(config.features.standalone_modules ? 'Yes' : 'No')}`)
        console.log(`  Modules Path: ${chalk.green(config.paths.modules)}`)
        console.log('')
        console.log(chalk.cyan('Next steps:'))
        console.log(`  1. Create your first module: ${chalk.green('misan create:module user-management')}`)
        console.log(`  2. Add components to modules: ${chalk.green('misan create:component login-form user-management')}`)
        console.log(`  3. List your modules: ${chalk.green('misan list modules')}`)
        console.log(`  4. Explore configuration: ${chalk.green('misan config show')}`)
...
async function createBasicDirectoryStructure(
  ...
  try {
    ...
    await ensureDirectoryExists(path.join(rootDir, projectStructure.framework.paths.shared))
    
    // Create tests directory if testing is enabled
    if (projectStructure.hasTesting) {
      await ensureDirectoryExists(path.join(rootDir, projectStructure.framework.paths.tests))
    }
    
    // Create gitkeep files to ensure directories are tracked
    await fs.writeFile(path.join(rootDir, modulesPath, '.gitkeep'), '')
    await fs.writeFile(path.join(rootDir, projectStructure.framework.paths.shared, '.gitkeep'), '')
    
    logInfo('Directory structure created successfully')
  } catch (error) {
    logError(`Failed to create directory structure: ${(error as Error).message}`)
    throw error
  }
}
