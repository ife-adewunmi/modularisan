    ...
    .action(async (name, moduleName, options) => {
      try {
        ...
        if (!name || !moduleName) {
          if (options.yes) {
            logError(
              'Component name and module are required when using --yes flag'
            );
            return;
          }
          ...
          const answers = await inquirer.prompt([
            {
              ...
              validate: (input) => {
                ...
                if (!validateName(input))
                  return 'Invalid component name. Use kebab-case (e.g., login-form)';
                return true;
              },
            },
            {
        ...
        // Success message
        logSuccess(
          `Component '${name}' created successfully in module '${moduleName}'!`
        );
        console.log('');
        ...
        console.log(
          `  Props: ${chalk.green(componentOptions.props ? 'Yes' : 'No')}`
        );
        console.log(
          `  Client-side: ${chalk.green(componentOptions.client ? 'Yes' : 'No')}`
        );
        console.log(
          `  Story: ${chalk.green(componentOptions.story ? 'Yes' : 'No')}`
        );
        console.log(
          `  Test: ${chalk.green(componentOptions.test ? 'Yes' : 'No')}`
        );
      } catch (error) {
