...
export function aiArchitectCommand(program: Command): void {
  program
    ...
    .option(
      '-o, --output <path>',
      'Output file path for generated architecture'
    )
    ...
    .action(
      async (requirements: string | undefined, options: AiArchitectOptions) => {
        try {
          // Load configuration
          ...
          if (options.interactive || !projectRequirements) {
            const answers = await inquirer.prompt<InteractiveAnswers>([
              ...
              {
                type: 'input', // Changed from 'textarea' as it's not a standard inquirer type
                name: 'description',
              ...
              },
              {
                ...
                choices: [
                  {
                    name: 'Small (1-5 developers, simple features)',
                    value: 'small',
                  },
                  {
                    name: 'Medium (5-15 developers, moderate complexity)',
                    value: 'medium',
                  },
                  {
                    name: 'Large (15+ developers, complex features)',
                    value: 'large',
                  },
                  {
                    name: 'Enterprise (Multiple teams, high complexity)',
                    value: 'enterprise',
                  },
                ],
              },
...
`;
          ...
          logInfo(
            'Analyzing requirements and generating architecture suggestions...'
          );

          // Get AI architecture suggestions
          const suggestions = await aiService.suggestArchitecture(
            projectRequirements || ''
          );
...
*Generated by Modularisan AI*`;
          ...
          if (options.createModules) {
            const { confirmCreate } = await inquirer.prompt<CreateConfirmation>(
              [
                {
                  type: 'confirm',
                  name: 'confirmCreate',
                  message:
                    'Would you like to create the suggested modules automatically?',
                  default: false,
                },
              ]
            );
            ...
            if (confirmCreate) {
              ...
              for (const moduleName of suggestions.modules) {
                try {
                  ...
                  logError(
                    `❌ Failed to create module '${moduleName}': ${(error as Error).message}`
                  );
                }
              }
          ...
          console.log(
            '2. Create modules manually or use --create-modules flag'
          );
