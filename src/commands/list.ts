    ...
    .action(async (type = 'modules', options) => {
      try {
        ...
        switch (type.toLowerCase()) {
          ...
          default:
            logError(
              `Unknown list type: ${type}. Available types: modules, frameworks, templates`
            );
            break;
        }
      } catch (error) {
...
async function listModules(
  ...
  if (modules.length === 0) {
    logInfo(
      'No modules found. Create your first module with: misan create:module <name>'
    );
    return;
  }
  ...
  modules.forEach((module, index) => {
    ...
    if (options.detailed) {
      ...
      console.log(
        `   Routing: ${module.hasRouting ? chalk.green('✓') : chalk.red('✗')}`
      );
      console.log(
        `   API: ${module.hasApi ? chalk.green('✓') : chalk.red('✗')}`
      );
      console.log(
        `     Tests: ${module.hasTests ? chalk.green('✓') : chalk.red('✗')}`
      );
      console.log(
        `     Standalone: ${module.isStandalone ? chalk.green('✓') : chalk.red('✗')}`
      );
      console.log('');
    }
  });
...
async function listSupportedFrameworks(options: any): Promise<void> {
  ...
  const grouped = frameworks.reduce(
    (acc, framework) => {
      const existing = acc[framework.type] ?? [];
      acc[framework.type] = [...existing, framework];
      return acc;
    },
  ...
  Object.entries(grouped).forEach(([type, items]) => {
    console.log(
      chalk.yellow(`${type.charAt(0).toUpperCase() + type.slice(1)}:`)
    );
    items.forEach((framework) => {
      console.log(
        `  ${chalk.green(framework.name)} - ${chalk.gray(framework.description)}`
      );
    });
    console.log('');
  });
...
async function listTemplates(options: any): Promise<void> {
  const templates = {
    module: [
      ...
      {
        name: 'full',
        description: 'All components including hooks, utils, tests',
      },
  ...
  Object.entries(templates).forEach(([category, items]) => {
    ...
    items.forEach((template) => {
      console.log(
        `  ${chalk.green(template.name)} - ${chalk.gray(template.description)}`
      );
    });
    console.log('');
  });
