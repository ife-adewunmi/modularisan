    ...
    .action(async (name, moduleName, options) => {
      try {
        ...
        if (!name || !moduleName) {
          if (options.yes) {
            logError(
              'Service name and module are required when using --yes flag'
            );
            return;
          }
          ...
          const answers = await inquirer.prompt([
            {
              ...
              validate: (input) => {
                ...
                if (!validateName(input))
                  return 'Invalid service name. Use kebab-case (e.g., user-service)';
                return true;
              },
            },
            {
        ...
        if (!options.yes) {
          ...
          serviceOptions = {
            server:
              answers.serviceType === 'server' ||
              answers.serviceType === 'universal',
            client:
              answers.serviceType === 'client' ||
              answers.serviceType === 'universal',
        ...
        const isServer =
          serviceOptions.server || config.framework.type === 'backend';
        ...
        // Success message
        logSuccess(
          `Service '${name}' created successfully in module '${moduleName}'!`
        );
        console.log('');
        ...
        console.log(
          `  Type: ${chalk.green(isServer ? 'Server-side' : 'Client-side')}`
        );
        console.log(
          `  API Templates: ${chalk.green(serviceOptions.api ? 'Yes' : 'No')}`
        );
        console.log(
          `  Test: ${chalk.green(serviceOptions.test ? 'Yes' : 'No')}`
        );
        console.log('');
