...
export function createFeatureCommand(program: Command): void {
  program
    ...
    .option(
      '-p, --path <path>',
      'Path where the feature folder will be created',
      'src/features'
    )
    ...
    .action(async (name, options) => {
      try {
          ...
          const answers = await inquirer.prompt([
            {
              ...
              validate: (input) => {
                ...
                if (!validateName(input))
                  return 'Invalid feature name. Use kebab-case (e.g., user-profile)';
                return true;
              },
            },
          ]);
        ...
        if (process.argv.length <= 3) {
          ...
          const answers = await inquirer.prompt([
            ...
            },
            {
              ...
              name: 'componentName',
              message:
                'Enter a component name to create (leave empty to skip):',
              validate: (input) => {
        ...
        console.log(
          chalk.cyan('  1.') + ' Add more components to your feature:'
        );
        console.log(
