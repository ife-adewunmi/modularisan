...
async function debugEnvironment(verbose: boolean): Promise<void> {
  ...
  for (const pm of packageManagers) {
    const lockExists = await fs.pathExists(path.join(process.cwd(), pm.lockFile))
    if (lockExists) {
      console.log(`  Package manager: ${chalk.green(pm.name)} (${pm.lockFile} found)`)
      break
    }
  }
...
async function debugConfiguration(): Promise<void> {
  ...
  try {
    ...
    if (!hasConfig) {
      ...
      console.log(`  ${chalk.yellow('💡 Run "misan init" to initialize the project')}`)
      console.log('')
      return
    }
    ...
    console.log(`  Framework: ${chalk.blue(config.framework.name)} (${config.framework.type})`)
...
async function debugProjectStructure(verbose: boolean): Promise<void> {
  ...
  try {
    ...
    console.log(`  package.json: ${packageJsonExists ? chalk.green('✓') : chalk.red('✗')}`)
    
    if (packageJsonExists) {
      const packageJson = await fs.readJson(packageJsonPath)
      console.log(`  Project name: ${chalk.blue(packageJson.name || 'unnamed')}`)
    ...
    try {
      ...
      console.log(`  Detected framework: ${chalk.green(projectStructure.framework.name)}`)
      ...
      for (const [name, pathValue] of Object.entries(paths)) {
        if (pathValue) {
          ...
          console.log(`  ${name} directory: ${exists ? chalk.green('✓') : chalk.yellow('✗')} ${chalk.gray(pathValue)}`)
        }
      }
...
async function debugTemplates(verbose: boolean): Promise<void> {
  ...
  try {
    ...
    console.log(`  Stubs directory: ${stubsExists ? chalk.green('✓') : chalk.red('✗')} ${chalk.gray(stubsDir)}`)
    ...
    if (stubsExists) {
      ...
      console.log(`  ${chalk.yellow('⚠️  Templates not found. Some features may not work correctly.')}`)
    }
...
async function debugModules(): Promise<void> {
  ...
  try {
    ...
    if (modules.length === 0) {
      console.log(`  ${chalk.yellow('💡 No modules found. Create your first module with "misan create:module <name>"')}`)
    } else {
...
async function listTemplateFiles(dir: string, prefix = ''): Promise<string[]> {
  ...
  for (const file of files) {
    ...
    if (stat.isDirectory()) {
      const subTemplates = await listTemplateFiles(filePath, `${prefix}${file}/`)
