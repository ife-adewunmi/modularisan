...
import { validateName, toPascalCase } from '@/utils/validators';
...
export class ModuleService {
  ...
  async createComponent(options: CreateComponentOptions): Promise<void> {
    ...
    if (!validateName(name)) {
      throw new Error(
        'Invalid component name. Use kebab-case (e.g., login-form)'
      );
    }
  ...
  async createService(
    ...
    if (!validateName(serviceName)) {
      throw new Error(
        'Invalid service name. Use kebab-case (e.g., user-service)'
      );
    }
  ...
  async listModules(): Promise<ModuleStructure[]> {
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        ...
        const moduleStructure = await this.analyzeModule(
          entry.name,
          modulePath
        );
        if (moduleStructure) {
  ...
  async deleteModule(name: string, force = false): Promise<void> {
    ...
    if (!force) {
      ...
      logInfo(
        `This will permanently delete the module '${name}' and all its contents.`
      );
    }
  ...
  private async createModulePackageJson(
    ...
    const packageJson = {
      ...
      peerDependencies: {
        ...(this.config.framework.name.includes('React') && {
          react: '^18.0.0',
          'react-dom': '^18.0.0',
        }),
  ...
  private async createStoryFile(
    name: string,
    componentDir: string
  ): Promise<void> {
    // Create Storybook story file
    const storyContent = await renderTemplate('common/story.ejs', {
      name,
      componentName: toPascalCase(name),
    });
  ...
  private async createComponentTestFile(
    name: string,
    componentDir: string
  ): Promise<void> {
    const testContent = await renderTemplate(
  ...
  private async createServiceTestFile(
    serviceName: string,
    servicesDir: string
  ): Promise<void> {
    const testContent = await renderTemplate(
  ...
  private async updateComponentIndex(
    componentDir: string,
  ...
  private async updateServicesIndex(
    servicesDir: string,
    serviceName: string
  ): Promise<void> {
  ...
  private async analyzeModule(
    ...
    try {
      ...
      const hasRouting =
        components.includes('pages') || components.includes('routes');
      const hasApi =
        components.includes('api') || components.includes('controllers');
      const hasTests =
        components.includes('tests') || components.includes('__tests__');
  ...
  private getDefaultComponents(template?: string): string[] {
    if (
      template &&
      MODULE_TEMPLATES[template as keyof typeof MODULE_TEMPLATES]
    ) {
  ...
  private async updateTypesIndex(
    typesDir: string,
    typeName: string
  ): Promise<void> {
