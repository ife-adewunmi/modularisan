...
export interface AIProvider {
  ...
  suggestArchitecture(requirements: string, context: AIContext): Promise<AIArchitectureSuggestion>
}
...
export class AIService {
  ...
  async suggestArchitecture(requirements: string): Promise<AIArchitectureSuggestion> {
    if (!this.provider) {
      throw new Error('AI provider not initialized')
    }
  ...
  private buildComponentPrompt(options: any, context: AIContext): string {
    const { name, description, moduleName, props = [], features = [] } = options
...
5. Test file structure if testing is enabled`
  ...
  private buildServicePrompt(options: any, context: AIContext): string {
    const { name, description, moduleName, methods = [], isServer = false } = options
