...
export class ConfigManager {
  ...
  async initializeConfig(
    ...
    options: Partial<ModularisanConfig> = {}
  ): Promise<ModularisanConfig> {
    const { framework, hasTypeScript, hasTesting, packageManager } =
      projectStructure;
  ...
  async loadConfig(): Promise<ModularisanConfig> {
    ...
    for (const legacyName of LEGACY_CONFIG_NAMES) {
      ...
      if (await fs.pathExists(legacyPath)) {
        logInfo(
          `Found legacy config file: ${legacyName}. Consider migrating to ${CONFIG_FILE_NAMES[0]}`
        );
        try {
          ...
          logSuccess(
            `Migrated configuration from ${legacyName} to ${CONFIG_FILE_NAMES[0]}`
          );

          return this.config!;
        } catch (error) {
          logError(
            `Failed to migrate legacy config: ${(error as Error).message}`
          );
        }
      }
    }
    ...
    throw new Error(
      `No configuration found. Run 'misan init' to initialize the project.`
    );
  }
  ...
  async updateConfig(
    updates: Partial<ModularisanConfig>
  ): Promise<ModularisanConfig> {
  ...
  async validateConfig(config?: ModularisanConfig): Promise<boolean> {
    ...
    const modulesPath = path.join(
      configToValidate.project.rootDir,
      configToValidate.paths.modules
    );
