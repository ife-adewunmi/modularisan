...
export class FeatureService {
  async createFeature(options: CreateFeatureOptions): Promise<void> {
    ...
    try {
      ...
      if (createIndex) {
        try {
          const indexContent = await renderTemplate("feature/index.ejs", { name })
          await fs.writeFile(path.join(featurePath, "index.ts"), indexContent)
          logInfo("Creating index.ts file")
        } catch (error) {
          logError(`Failed to create index.ts file: ${(error as Error).message}`)
          // Fallback to a simple index file
          const fallbackContent = `// Export all components from this feature\nexport * from './components'\n`
          await fs.writeFile(path.join(featurePath, "index.ts"), fallbackContent)
      ...
      if (componentName && componentName.length > 0) {
        if (components.includes("components")) {
          try {
            const componentContent = await renderTemplate("component/functional.ejs", {
              name: componentName,
              withProps: true,
              isClientComponent: true,
            })

            await fs.writeFile(path.join(featurePath, "components", `${componentName}.tsx`), componentContent)
            logInfo(`Creating component: ${componentName}.tsx`)

            // Create index.ts in components directory
            try {
              const componentsIndexContent = await renderTemplate("component/index.ejs", {
                name: componentName,
              })

              await fs.writeFile(path.join(featurePath, "components", "index.ts"), componentsIndexContent)
              logInfo("Creating components/index.ts file")
            } catch (error) {
              logError(`Failed to create components/index.ts file: ${(error as Error).message}`)
              // Fallback to a simple index file
              const fallbackContent = `export * from './${componentName}'\n`
              await fs.writeFile(path.join(featurePath, "components", "index.ts"), fallbackContent)
