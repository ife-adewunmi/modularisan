...
export class ComponentService {
  async createComponent(options: CreateComponentOptions): Promise<void> {
    ...
    try {
      ...
      try {
        const templateName =
          type === 'class' ? 'component/class.ejs' : 'component/functional.ejs';
        ...
        logError(
          `Failed to create component file: ${(error as Error).message}`
        );
...
`;
      ...
      if (withStyle) {
        try {
          const styleContent = await renderTemplate('component/style.ejs', {
            name,
          });
          await fs.writeFile(
...
`;
      ...
      if (withTest) {
        try {
          const testContent = await renderTemplate('component/test.ejs', {
            name,
          });
      ...
      if (withStory) {
        try {
          const storyContent = await renderTemplate('component/story.ejs', {
            name,
          });
          await fs.writeFile(
